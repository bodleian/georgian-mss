...
if [ ! "$5" == "noindex" ]; then
    ...
    if [ ! $1 == "organizations.xquery" ]; then
        ...
        curl -X POST -fsS "http://${4}:8983/solr/georgian-mss/update?commit=true" --data-binary "<delete><query>type:${3}</query></delete>" -H "Content-Type: text/xml" 1>> $LOGFILE 2>> $LOGFILE
    fi
    ...
    if [ $? -gt 0 ]; then
        ...
        curl -fsS "http://${4}:8983/solr/georgian-mss/update?commit=true" --data-binary @solr/$2 -H "Content-Type: text/xml" 1>> $LOGFILE 2>> $LOGFILE
        if [ $? -eq 0 ]; then
