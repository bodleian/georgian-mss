...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Save the response from the following Solr query run on whichever Digital Bodleian core is the current production copy:
         http://<solr_server>:<solr_port>/solr/<core_name>/select?fl=id,shelfmark_s,surface_count_i,completeness_s&fq=all_collections_id_sm%3Ageorgian&q=*%3A*&rows=9999&wt=xml-->
    <xsl:variable name="dbsolrresponse" as="document-node()" select="document('/tmp/georgian_db.xml')"/>
