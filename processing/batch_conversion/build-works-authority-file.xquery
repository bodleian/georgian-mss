            ...
            <listBibl>
{
    ...
    let $allworks as element()* := (
        ...
        for $msitem in $collection//tei:msItem[@xml:id]
            ...
            (: No "alt"-type titles in Georgian: let $alttitles as xs:string* := (for $t in $msitem/title[@type='alt'] return normalize-space(string-join($t//text(), ' ')))[string-length(.) gt 0][not(. = $notitletitles)] :)
            let $alttitles as xs:string* := ()
            return
            if (count($titles) eq 0 and not($msitem/tei:author)) then
                ...
                (: In Georgian there are sometimes multiple titles in individual works :)
                <work>
                    {
                    for $title in $titles
                        return (
                    ...
                    {
                    (: No "alt"-type titles in Georgian but leaving this in here for future reference :)
