...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="cat" select="'Georgian'"/>
  ...
  <xsl:variable name="path">
    <xsl:value-of
      select="concat('file:///home/jamesc/git/georgian-mss/working/new/?select=', $files,';on-error=warning;recurse=',$recurse)"
    />
  </xsl:variable>
  ...
  <xsl:template name="main">
    ...
    <xsl:for-each select="$doc">
      ...
      <xsl:variable name="outputFilename"
        select="concat('file:///home/jamesc/git/georgian-mss/working/draft-updated/', $msID, '.xml')"/>
      <!-- create output file -->
...
<xsl:choose>
  ...
  <!-- 
  Replace publicationStmt more carefully in georgian
  -->
          ...
          <revisionDesc>
            <change when="{substring(string(current-date()), 0, 11)}">
              <persName>James Cummings</persName> Up-converted the markup using <ref
                target="https://github.com/bodleian/georgian-mss/tree/master/processing/convertGeorgian2Bodley.xsl"
                >https://github.com/bodleian/georgian-mss/tree/master/processing/convertGeorgian2Bodley.xsl</ref>
            </change>
          </revisionDesc>
        </xsl:otherwise>
      </xsl:choose>
